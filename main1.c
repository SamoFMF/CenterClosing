...
int main(int argc, char* argv[]) {
    ...
    char* algorithm_names[NUM_ALGS] = { "bnb", "bf", "bt", "btd", "sc", "bnb+", "hs", "prand", "pfirst", "pclosest", "prand+", "pfirst+", "pclosest+", "hrand", "hbest", "h+", "greedy", "card", "cardb", "radius", "radiusb", "greedyb" };
    ...
    for (int i = 0; i < iterations; i++) {
        if (input_iterate == 0) {
            ...
            sprintf_s(iterations_string, 16, "%d", i + 1);
        ...
        filename_out = concat(concat(folder, "results/", len_folder), filename_out, len_folder + 8);
        ...
        fprintf(file, "{\"data\": \"info\", \"n\": %d, \"m\": %d, \"krange\": [%d, %d, %d], \"algorithms\": [", G->n, G->m, kstart, kstep, kend);
        int count_algs = 0;
        for (int i = 0; i < NUM_ALGS; i++)
            if (bitset_contains(algorithms, i)) {
